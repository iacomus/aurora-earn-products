...
const strategiesEnvelope = (...items: object[]) => ({
  error: [],
  result: { items },
});
const assetsEnvelope = (assetsObj: object) => ({
  error: [],
  result: assetsObj,
});
...
describe("GET /earn-products", () => {
  it("returns instant-access products only for tier=standard", async () => {
    ...
    const res = await request(createApp(new FileMockMeridianClient(dir))).get(
      "/earn-products?tier=standard",
    );
    expect(res.status).toBe(200);
    expect(res.body.map((p: { strategyId: string }) => p.strategyId)).toEqual([
      "S-INSTANT",
    ]);
  });
  ...
  it("500s with a structured error when the data directory is missing", async () => {
    const res = await request(
      createApp(new FileMockMeridianClient(path.join(dir, "absent"))),
    ).get("/earn-products?tier=premium");
  ...
  it("returns an empty array when no product qualifies for the tier", async () => {
    await write(
      "strategies.json",
      strategiesEnvelope(bonded("S-BONDED", "SOL", "5.0000")),
    );
    await write(
      "assets.json",
      assetsEnvelope({ SOL: { altname: "SOL", status: "enabled" } }),
    );
    const res = await request(createApp(new FileMockMeridianClient(dir))).get(
      "/earn-products?tier=standard",
    );
